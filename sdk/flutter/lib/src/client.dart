...
class IngestaoVetorialClient {
  ...
  Map<String, String> _headersWith(Map<String, String>? extraHeaders,
      {bool jsonBody = false}) {
  ...
  Future<Uint8List> _requestBytes(String path,
  ...
  List<T> _extractItems<T>(dynamic payload, T Function(JsonMap json) decoder) {
    if (payload is List) {
      return payload
          .map((item) => decoder(_asJsonMap(item)))
  ...
  Future<List<T>> _getAllItems<T>(String path, T Function(JsonMap json) decoder,
    ...
    while (true) {
      final payload =
          await _requestJson('GET', path, queryParameters: <String, Object?>{
        ...?queryParameters,
        'skip': skip,
        'limit': limit,
      });
  ...
  Future<T> _patchObject<T>(
      String path, T Function(JsonMap json) decoder, Object body) async {
  ...
  Future<List<EmbeddingModelOption>> embeddingModels() {
    return _getItems(
        '/api/v1/collections/embedding-models', EmbeddingModelOption.fromJson);
  }

  Future<List<Collection>> collections(
      [CollectionListParams params = const CollectionListParams()]) {
  ...
  Future<Collection> createCollection(CreateCollectionParams params) {
    return _postObject('/api/v1/collections', Collection.fromJson,
        body: params.toJson());
  }
  ...
  Future<Collection> updateCollection(
      String collectionId, UpdateCollectionParams params) {
    return _patchObject('/api/v1/collections/$collectionId',
        Collection.fromJson, params.toJson());
  }

  Future<void> deleteCollection(String collectionId) =>
      _delete('/api/v1/collections/$collectionId');

  Future<JsonMap> collectionRaw(String collectionId) async {
    final payload =
        await _requestJson('GET', '/api/v1/collections/$collectionId/raw');
  ...
  Future<List<Document>> collectionDocuments(String collectionId,
    ...
    return _getItems(
        '/api/v1/collections/$collectionId/documents', Document.fromJson,
        queryParameters: <String, Object?>{'skip': skip, 'limit': limit});
  }

  Future<List<Document>> documents(
      [DocumentListParams params = const DocumentListParams()]) {
  ...
  Future<List<DocumentChunk>> documentChunks(String documentId,
    ...
    return _getAllItems(
        '/api/v1/documents/$documentId/chunks', DocumentChunk.fromJson,
  ...
  Future<void> deleteDocument(String documentId) =>
      _delete('/api/v1/documents/$documentId');
  ...
  Future<UploadResponse> reprocessDocument(String documentId,
    ...
    return _postObject(
        '/api/v1/documents/$documentId/reprocess', UploadResponse.fromJson,
  ...
  Future<DocumentDetail> setVersionActive(
      String documentId, int version, bool isActive) {
    return _patchObject(
  ...
  Future<UploadResponse> upload(UploadFile file, UploadOptions options) async {
    ...
    request.fields['metadata'] =
        jsonEncode((options.metadata ?? const UploadMetadata()).toJson());
    request.fields['overwrite_existing'] =
        (options.overwriteExisting ?? false).toString();
  ...
  Future<List<String>> tags(
      [TagListParams params = const TagListParams()]) async {
    final payload = await _requestJson('GET', '/api/v1/tags',
        queryParameters: params.toQueryParameters());
    if (payload is List) {
  ...
  Future<List<String>> searchTags(String query) async {
    final payload = await _requestJson('GET', '/api/v1/tags/search',
  ...
  Future<Tag> createTag(String name) {
    return _postObject('/api/v1/tags', Tag.fromJson,
  ...
  Future<JobProgress> jobProgress(String documentId, int version) {
    return _getObject(
        '/api/v1/progress/$documentId/versions/$version', JobProgress.fromJson);
  }
  ...
  Future<Uint8List> exportLogs(
      [LogExportParams params = const LogExportParams()]) {
    return _requestBytes('/api/v1/logs/export',
        queryParameters: params.toQueryParameters());
  }

  Future<LogIngestResponse> ingestLogs(List<LogIngestItem> payload,
      {String? logSinkToken}) {
    return _postObject('/api/v1/logs/ingest', LogIngestResponse.fromJson,
        body: payload.map((item) => item.toJson()).toList(growable: false),
        extraHeaders: logSinkToken == null
            ? null
            : <String, String>{'X-Log-Sink-Token': logSinkToken});
  }

  Future<TokenUsageList> tokenUsage(
      [TokenUsageParams params = const TokenUsageParams()]) {
